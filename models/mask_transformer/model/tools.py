...
def gumbel_softmax(logits, temperature=1., hard=True, dim=1):
    z = logits + gumbel_noise(logits)
    y_soft = F.softmax(z / max(temperature, 1e-10), dim=dim)
...
def softmax(logits, tau=1., hard=True, dim=1):
    y_soft = F.softmax(logits / max(tau, 1e-10), dim=dim)
