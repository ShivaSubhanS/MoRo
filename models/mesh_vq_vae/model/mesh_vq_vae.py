...
from .graphAE import FullyConvAE
...
class MeshVQVAE(nn.Module):
    def __init__(
        ...
        self.fully_conv_ae = FullyConvAE(cfg.meshconv)

        self.encoder = Encoder(self.fully_conv_ae)

        num_embeddings = cfg.num_embeddings
        embedding_dim = cfg.embedding_dim
        decay = cfg.decay
        num_quantizer = cfg.num_quantizer
        ...
        self.decoder = Decoder(self.fully_conv_ae)
    ...
    def forward(self, x, detailed=False):
        ...
        z = self.encoder(x)
        if self.quantize:
            ret = self.vq_vae(z, detailed=detailed)
            if not detailed:
                ret["x_recon"] = self.decoder(ret["quantized"])
                # x_recon = self.decoder(quantized)
            else:
                x_recon = []
                for quantized_int in ret["quantized"]:
    ...
    def get_codebook(self):
        ...
        """
        return self.vq_vae.get_codebook()
