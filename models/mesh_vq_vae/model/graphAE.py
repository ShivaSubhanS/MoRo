...
class FullyConvAE(nn.Module):
        ...
        for layer_name in self.connection_layer_lst:
            ...
            for fn in fn_lst:
                if (layer_name in fn) and ((".npy" in fn) or (".npz" in fn)):
                    self.connection_layer_fn_lst += [
                        os.path.join(self.connection_folder, fn)
                    ]
                    find_fn = True
                    break
            if find_fn == False:
    ...
    def init_layers(self):
        ...
        for l in range(self.layer_num):
            ...
            # self.register_buffer("neighbor_num_lst" + str(l), neighbor_num_lst)
            # neighbor_num_lst = self.getattr("neighbor_num_lst" + str(l))
            ...
            # self.register_buffer("neighbor_id_lstlst" + str(l), neighbor_id_lstlst)
            # neighbor_id_lstlst = self.getattr("neighbor_id_lstlst" + str(l))
            ...
            if residual_rate < 1:
                weights = torch.randn(weight_num, out_channel * in_channel)
    ...
    def init_test_mode(self):
        for l in range(len(self.layer_lst)):
            ...
            if len(conv_layer) != 0:
                ...
                conv_layer = weights.cuda(), bias.cuda()
            ...
            if len(residual_layer) != 0:
                ...
                if in_pn != out_pn:
                    ...
                    residual_layer = weight_res, p_neighbors.cuda()
            ...
            self.layer_lst[l] = (
                ...
                max_neighbor_num,
                neighbor_num_lst,
                neighbor_id_lstlst.cuda(),
                conv_layer,
                residual_layer,
                residual_rate,
                neighbor_mask_lst,
                zeros_outpn_outchannel.cuda(),
            )
    ...
    def forward_one_conv_layer_batch_during_test(
        ...
        in_pc_pad = torch.cat(
            (in_pc, torch.zeros(batch, 1, in_channel, device=device)), 1
        ...
        out_pc_conv = zeros_outpn_outchannel.clone().to(device)
        ...
        #    return out_pc
        ####compute output of residual layer####
        out_pc_res = zeros_outpn_outchannel.clone()
    ...
    def forward_one_conv_layer_batch(self, in_pc, layer_info, is_final_layer=False):
        ...
        neighbor_id_lstlst = neighbor_id_lstlst.to(in_pc.device)
        neighbor_mask_lst = neighbor_mask_lst.to(in_pc.device)
        zeros_outpn_outchannel = zeros_outpn_outchannel.to(in_pc.device)

        in_pc_pad = torch.cat(
            (in_pc, torch.zeros(batch, 1, in_channel).to(in_pc)), 1
        )  # batch*(in_pn+1)*in_channel
        ...
        ####compute output of convolution layer####
        out_pc_conv = zeros_outpn_outchannel.clone()
